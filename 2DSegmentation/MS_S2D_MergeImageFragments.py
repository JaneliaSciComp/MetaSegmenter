...
def process_inputs_zmerge(zdim, input_label, node, dict_nodes_zmerge, options):
    node      = int(node)
    match_str = dict_nodes_zmerge[node]

    output_path = os.path.join(ms_data, input_label + "_" + match_str + ".h5")
    ...
    if options.verbose:
        print "command_zmerge=", command_zmerge
    command_rm     = "rm -f "
    z_range = range(max(0, int(options.zmin)), min(zdim, int(options.zmax)))
    for z in z_range:
        if match_str in ["_Seg.png", "_RGB.png"]:
            input_file = input_label + "_z" + str(z+1) + match_str + ".png"
        else:
            input_file = input_label + "_z" + str(z+1) + match_str + ".png"
...
def map_nodes_zmerge(zdim, options):
    ...
    dict_nodes_zmerge = {}
    dict_nodes_zmerge[1] = "_BW.png"
    dict_nodes_zmerge[2] = "_Seg.png"
    dict_nodes_zmerge[3] = "_RGB.png"
