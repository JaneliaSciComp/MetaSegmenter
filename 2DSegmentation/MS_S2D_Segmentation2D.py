...
def segmentation_command_line_parser(parser):
    ...
    parser.add_option("-f", "--fracBlack", dest="fracBlack", help="fracBlack for detecting neurons",metavar="fracBlack", default=0.35)
    parser.add_option("-F", "--fracBlack2",dest="fracBlack2",help="fracBlack for detect. dark str.",metavar="fracBlack2",default=0.65)
    ...
    parser.add_option("-P", "--processing_step",dest="processing_step",help="start processing from this step ",metavar="processing_step",default=1)
    parser.add_option("-s", "--sub",dest="submission_command", help="source, qsub or qsub_debug", metavar="submission_command", default="qsub")
    parser.add_option("-S", "--slots",dest="num_slots", help="# of cluster slits per 1 job", metavar="num_slots", default=1)
...
def get_input_type(input_data, options):
    input_type = ""
    if os.path.isfile(os.path.join(ms_data,input_data)) or \
       os.path.isfile(os.path.join(ms_temp,input_data)):
        input_type = "file"
    elif os.path.isdir(os.path.join(ms_data,input_data)) or \
         os.path.isdir(os.path.join(ms_temp,input_data)):
        input_type = "directory"
    elif is_DVID_subvolume(input_data, options):
        input_type = "DVID_subvolume"
    if options.verbose:
        print "The type of input_data:", input_data, " is: ", input_type
    return input_type
...
#                       image_shape = misc.imread(myfile).shape
    ...
    elif input_type == "DVID_subvolume":
        ranges = input_data[1:len(input_data)-1].split(",")
        for i in range(0,3):
...
def create_data_extraction_and_segmentation_job(outfolderpath, \
    ...
    fragment_segmentation_script_path = \
        os.path.join(outfolderpath, "Segmentation_script." +\
                     input_label + ".sh")
    executable_extr = os.path.join(ms_home, "2DSegmentation", \
                                   "MS_S2D_ExtractInputData.py")
    executable_segm = os.path.join(ms_home, "2DSegmentation", \
    ...
    command_segm_data = \
        ...
        " -f " + options.fracBlack + " -F "+ options.fracBlack2 
    if options.verbose:
        command_extr_data += " -v "
        command_segm_data += " -v "
    if options.debug:
...
def process_input_data_low_level(dict_node_xyz, input_label,options):
    ...
    command_segm = executable_path + " " + input_file + " " + \
                   str(options.fracBlack) + " " + str(options.fracBlack2) + " " +\
...
if __name__ == "__main__":
    ...
    if len(args) == 1:
        ...
        if input_type in ["file", "directory", "DVID_subvolume"]:
